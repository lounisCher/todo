...
const TaskCol = ({ name, img, tasks, status, handleDelete, editTask}) => {
  return (
    <section className="border border-slate-800 w-3/4 h-screen m-3 rounded-md shadow-lg flex flex-col">
      ...
      scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-slate-200 scrollbar-thumb-rounded">
        {tasks.map(
          (task, index) =>
            task.status === status && (
              <TaskCard key={index} title={task.task} tags={task.tags} handleDelete={handleDelete} index={index} editTask={editTask} />
            )
        )}
      </div>
    </section>
